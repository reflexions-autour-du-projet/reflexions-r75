...
const App = () => {
  ...
  const renderContentItem = (item, key) => {
    ...
    // { periode, intro/detail, elements/moments/comparaison/mutations, heritage/note }
    if (item.periode && (item.intro || item.detail || item.moments || item.elements || item.comparaison || item.mutations)) {
    ...
    if (item.titre && (item.evenements || item.dynamiques || item.exemples || (item.formes && !item.mecanisme))) {
      return (
